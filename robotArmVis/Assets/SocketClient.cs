...
public class SocketClient : MonoBehaviour {
	...
	void MakeRobotModel(float[] pos){
		...
		for (int i = 0; i < 2; i++) {
			...
			m_finger[i].transform.localScale = new Vector3(8.0f, length, 5.0f);
		...
		m_finger[2].transform.localScale = new Vector3(8.0f, length, 8.0f);
	...
	void MoveRobotModel(float[] pos){
		...
		for (int i = 0; i < 2; i++) {
			...
			m_finger[i].transform.localScale = new Vector3(8.0f, length, 8.0f);
			m_finger[i].transform.up = (fingerPos[i] - proj).normalized;
		}
